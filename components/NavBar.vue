...
<template>
  ...
  <nav
    class="grow-1  bottom-0 px-[1.25rem] py-[0.75rem] sm:py-8 rounded-t-[1.5rem] sm:rounded-r-[1.5rem] w-screen sm:w-[100px] lg:w-[252px] bg-dark-500 overflow-hidden z-50">
    ...
    <ul class="flex sm:flex-col gap-4 sm:mt-12 sm:w-[220px] md:h-5/6 overflow-hidden z-10">
      <li v-for="{ icon }, route in routes" :key="route" :ref="route" @click="route === 'logout' ? onLogout() : null"
        ...
        <NuxtLink :to="route === 'logout' ? '/' : `/${route}`" :replace="route !== 'dashboard'"
          class="flex items-center p-2 sm:py-2 gap-2 rounded-2xl font-semi-bold bg-dark-500 overflow-hidden opacity-50 cursor-pointer"
          :class="{ 'justify-self-center self-end': route === 'logout' }"
          active-class="bg-dark-600 text-primary-400 transition-colors duration-500 opacity-100">
          <svg v-if="route !== 'logout'" width="21" height="32" viewBox="0 0 21 32" fill="none"
            xmlns="http://www.w3.org/2000/svg" class="hidden sm:inline">
